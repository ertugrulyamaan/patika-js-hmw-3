...
update()
function createFood({id,title,category,price,img,desc}){
  if (category.toLowerCase() == filter || filter == "all"){
    ...
    money.classList.add("food-price")
    money.innerHTML=price
    
    mtitle.append(foodname,money)
    info.append(mtitle,description)
    item.append(pic,info)
    sectionDOM.append(item)
  }
}

function update() {
  for (let index = 0; index < menu.length; index++) { 
      createFood({...menu[index]})
  }
}
